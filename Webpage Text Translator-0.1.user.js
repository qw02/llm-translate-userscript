...
const PROVIDER_CONFIGS = {
    OPENROUTER : {
        ...
        S2_OPTIONS: [
            ...
            { value: {'model': 'google/gemini-2.0-flash-001', 'providers':['Google AI Studio']}, label: 'Gemini Flash 2.0' },
            { value: {'model': 'openai/gpt-4o-mini', 'providers':['OpenAI']}, label: '4o-mini' },
  ...
  "entries": [
    {
      ...
      "value": "[character] Name: John Doe (名無しの権兵衛) | Gender: Male | Nickname: Nanashi (ななし) | Notes: Lorem ipsum dolor sit amet ..."
    },
    {
      "keys": ["アメリカ合衆国", "アメリカ"],
      "value": "[location] Name: United States (アメリカ) | Notes: Consectetur adipiscing elit ..."
    }
  ]
}
...
/*
 * Parallel dictionary update implementation utilizing disjoint set partitioning.
 * New entries are first grouped by key intersection - entries sharing any keys (directly or transitively)
 * are placed in the same group. This creates independent partitions that can be processed in parallel.
 * Within each group:
 * 1. If the group has no intersection with existing dictionary entries, all entries can be processed
 *    in parallel as they represent completely new information
 * 2. If there are intersections, entries are processed sequentially as each merge operation may affect
 *    the context for subsequent merges
 */
async function updateDictionary(existingDict, newUpdates) {
    const systemPrompt = `You are a dictionary entry merger for a translation system using a RAG pipeline. Your task is to analyze existing entries and new updates, then output a merged dictionary that either updates existing entries or creates new ones with appropriate IDs while maintaining strict consistency in translations.
...
`;
    ...
    // Group entries by their key intersections
    const entryGroups = groupEntriesByIntersection(newUpdates.entries);

    const translationQueue = new TranslationQueue();
    translationQueue.setProgressDisplay(progressDisplay, newUpdates.length);

    const maxId = Math.max(...existingDict.entries.map(entry => entry.id), 0);

    // Process all groups in parallel
    const groupPromises = entryGroups.map(group => processEntryGroup(group, updatedDict, systemPrompt, maxId, translationQueue));

    await Promise.all(groupPromises);
    return updatedDict;
}

// Group entries that share keys with each other
function groupEntriesByIntersection(entries) {
    const groups = [];
    ...
    for (const entry of entries) {
        ...
        // Find all entries that share any keys with the current group
        let foundNew;
        do {
            foundNew = false;
            for (const otherEntry of entries) {
                if (processedEntries.has(otherEntry)) continue;

                const hasIntersection = otherEntry.keys.some(key => keySet.has(key));
                if (hasIntersection) {
                    group.add(otherEntry);
                    otherEntry.keys.forEach(key => keySet.add(key));
                    processedEntries.add(otherEntry);
                    foundNew = true;
                }
            }
        } while (foundNew);
...
async function processEntryGroup(group, updatedDict, systemPrompt, maxId, translationQueue) {
    // Find all existing entries that might be affected by this group
    const allKeys = new Set(group.flatMap(entry => entry.keys));
    const relevantEntries = findRelevantEntries(updatedDict.entries, group);

    // If no conflicts, process all entries in parallel
    if (!relevantEntries.length) {
        const promises = group.map(entry =>
                                   translationQueue.enqueueTask(async () => {
            processDictionaryMutation(updatedDict, entry);
        })
                                  );
    ...
    // Process entries sequentially if they have conflicts
    for (const entry of group) {
        await translationQueue.enqueueTask(async () => {
            const currentRelevantEntries = findRelevantEntries(updatedDict.entries, [entry]);

            if (!currentRelevantEntries.length) {
                processDictionaryMutation(updatedDict, entry);
                return;
            }
            ...
            const prompt = {
                ...
                user: constructUserPromptDictMerger(
                    { entries: currentRelevantEntries },
                    { entries: [entry] },
                    maxId
                ),
                assistant: `{"entries": [`
            ...
            try {
                ...
                for (const mergedEntry of mergedEntries.entries) {
                    const originalEntry = currentRelevantEntries.find(e => e.id === mergedEntry.id);
                    if (originalEntry) {
...
function extractJsonFromLLMOutput(output, prefill = '{"entries": [') {
    ...
    if (!trimmedOutput.startsWith(prefill)) {
        output = prefill + output.trimStart();
        console.warn('Assistant prefill was missing and has been added.');
    }
    try {
        const parsed = JSON.parse(output);
        if (parsed.entries && Array.isArray(parsed.entries)) {
            // Check if the entries array contains a single entry with its own entries array (common duplication case)
            if (parsed.entries.length === 1) {
                const firstEntry = parsed.entries[0];
                if (firstEntry.entries && Array.isArray(firstEntry.entries)) {
                    parsed.entries = firstEntry.entries;
                    console.warn('Nested entries detected and fixed.');
                }
            }
        }
        return parsed;
        ...
        if (match) {
            ...
            try {
                console.warn('Warning: JSON was extracted from a potentially malformed reply. Original output: ' + output);
                const parsed = JSON.parse(jsonString);
                // Check again for nested entries after extraction
                if (parsed.entries && Array.isArray(parsed.entries) && parsed.entries.length === 1) {
                    const firstEntry = parsed.entries[0];
                    if (firstEntry.entries && Array.isArray(firstEntry.entries)) {
                        parsed.entries = firstEntry.entries;
                        console.warn('Nested entries detected and fixed in extracted JSON.');
                    }
                }
                return parsed;
            } catch (jsonError) {
                console.error('Error parsing extracted JSON:', jsonError, 'Original output:', output);
                throw new Error('Failed to parse JSON from the output');
            }
        } else {
...
function init() {
    ...
    floatingBox.style.boxShadow = '0px 0px 10px rgba(0,0,0,0.1)';
    floatingBox.style.display = 'flex';
    floatingBox.style.flexDirection = 'column';
    floatingBox.style.gap = '5px';
